...
This book is an introduction to chip design using open-source tools. The topic of the book is the steps that are needed to produce a chip for a design described in a hardware description language (HDL) down to the files that are sent to the fab. Those steps are often called the backend design.
...
Albertos keynote: \cite{tides:eda}


\section{Tool Installation}

The magic of open-srouce tools is that we can install them on a personal computer without any concerns of licensing and license servers. The downside of open-source tools is the veriaety of installation options. There is no single best way for the installation. Furthermore, open-source tools are best supported on a Unix operating system, and preferable Linux. Support under MacOS comes second. MS Windows is best served by using the Window subsystme for Linux.

\subsection{Nix Based}

\subsection{Docker Images}

\subsection{Compileing from Source}

\section{Hello World}

After installing the tools, we want to see them in action. Synthesize a minimal circuit from the description in an HDL to the generation of a GDSII file and visualize it in an editor.
...
\section{Open-Source Production Frameoworks}
